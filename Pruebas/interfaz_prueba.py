...
class InterfazGrafica(tk.Tk):
    ...
    def iniciar_programa(self):
        if not self.configuracion_realizada_parte1:
            messagebox.showerror("Error", "Debe configurar las Coordenadas del Almacén.")
            return
        if not self.configuracion_realizada_parte2:
            messagebox.showerror("Error", "Debe configurar los Vehículos.")
            return
    ...
    def iniciar_programa_cantidad_usuarios(self):
        ...
        for i in range(clientes):
            label1 = ttk.Label(self.frame_iniciar_programa_cantidad_clientes, text=f"Ingrese las coordenadas (X, Y) del cliente {i + 1}:")
    ...
    def guardar_coordenadas_clientes(self):
        ...
        for i, (coord_x, coord_y) in enumerate(zip(coordenadas_x_clientes, coordenadas_y_clientes)):
            x_str, y_str = coord_x.get(), coord_y.get()
            coords = x_str.split(','), y_str.split(',')

            if len(coords[0]) != 1 or len(coords[1]) != 1 or \
               not coords[0][0].strip().replace('.', '', 1).lstrip('-').isdigit() or \
               not coords[1][0].strip().replace('.', '', 1).lstrip('-').isdigit():
                messagebox.showerror("Error", f"Ingrese coordenadas válidas para el cliente {i + 1}.")
                return

            x_float = float(coords[0][0].strip())
            y_float = float(coords[1][0].strip())
    ...
    def configuracion_programa(self):
        ...
        self.btn_configuracion_almacen.grid
    ...
    def configuracion_programa_vehiculos_cantidad_peso(self, cantidad):
        ...
        self.btn_guardar_configuracion_vehiculos_pesos.grid(row=vehiculos+1, column=1, pady=20)
        self.btn_regresar_sub_menu2.grid(row=vehiculos+2, column=1, pady=10)
