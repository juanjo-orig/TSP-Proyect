...
class InterfazGrafica(tk.Tk):
    ...
    def mapa(self):
        ...
        distancia_total = self.calcular_distancia_entrega(G, ruta_prim)

        # Mostrar la distancia total en la etiqueta
        self.label_ruta_entrega.config(text=f"Ruta de Entrega: {' -> '.join(ruta_prim)}\n"
                                            f"Distancia total: {distancia_total:.2f} unidades")

        distancia_total = 0  # Esto puede eliminarse, ya que se calcula en la línea anterior
