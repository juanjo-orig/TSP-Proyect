...
def dibujar_ruta_folium(mapa, ruta, posiciones, etiquetas, color):
    for nodo in ruta:
        folium.Marker(location=posiciones[nodo], popup=etiquetas[nodo], icon=folium.Icon(color=color)).add_to(mapa)
...
def mapa():
    ...
    etiquetas = {}  # Nuevo diccionario para almacenar nombres de los nodos
    ...
    for cliente, coord_cliente in clientes.items():
        ...
        etiquetas[cliente] = cliente  # Asignar el nombre del cliente como etiqueta
    posiciones["Almacén"] = almacen
    etiquetas["Almacén"] = "Almacén"  # Etiqueta para el almacén
    ...
    dibujar_ruta_folium(mapa, ruta_prim, posiciones, etiquetas, color='blue')
    ...
    dibujar_ruta_folium(mapa, ruta_completa, posiciones, etiquetas, color='green')
